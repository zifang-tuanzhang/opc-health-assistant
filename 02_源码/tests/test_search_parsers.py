...
def main() -> int:
    ...
    # 代价不对称：漏判 → 把验证页当搜索结果 → 产出编造来源（踩红线，不可接受）；
    ...
    # 动因：本机/评审机残留代理或 VPN 出口时，请求被带到非预期出口 —— 检索会被引擎当
    ...
    try:
        ...
        _hc.close()
    # 活体探针（最强判据）：死代理环境下，直连工厂造的客户端必须仍能连上本机服务。
    ...
    #   端到端实测被拖到 62.9 秒（评审会误判成"卡死"）。故移出默认列表，仅保留注册。
