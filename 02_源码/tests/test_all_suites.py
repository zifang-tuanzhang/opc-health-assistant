# -*- coding: utf-8 -*-
"""pytest 入口：一条 `pytest tests/` 即跑完离线七套确定性测试。

设计说明见同目录 conftest.py。这里刻意只跑**离线七套**：
    冒烟 / 护栏 / 8 组验收 / 检索层解析与降级 / 前端结构冒烟 / 澄清轮回归 / 运行保障与网关边界
...
def test_offline_suites() -> None:
    """离线七套必须全过（冒烟 / 护栏 / 8 组验收 / 检索层解析 / 前端结构冒烟 / 澄清轮回归 / 运行保障与网关边界）。"""
