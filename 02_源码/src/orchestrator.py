...
（analyze / search / generate / guardrail / done），供前端流式展示，让用户
**看得见**「真的在检索、真的在护栏打回」，而不是只等一个结果。

设计要点：
  反模式：「代码发现违规 → 代码直接写答案」（模型被架空）；
  正确做法：「代码发现违规 → 代码指出违规点 → 模型自行修正」（模型始终是控制器）。
  护栏只查「结构/契约/边界词」（左侧），不判断事实真假（右侧）。
...
#   · 旧实现（缺陷 #15）用 25 个词的词表 + 固定话术模板「请问您要查哪个城市或地区？」规定模型怎么说话，
#     且把"地区"当成必须缺失的条件——但本作品演示范围固定为昆明（UI 已声明），
#     问"你在哪个城市"既多余、又会在用户答"北京"时答不了（已二次批评）。
#   · 新实现（本处）：不写任何"说哪句话"的词表或固定话术，只给**事实**与**判断原则**：
...
#     ——这是"给判断 / 给声明"而非"写死话术"：用户或模型换任何说法都不影响判定。
...
#   层返回空，产品只说一句"暂未查到"——用户会误以为是检索功能故障。
#   故在降级时补一条**可点击的手动检索入口**：
...
def run_turn(
    ...
    try:
        ...
        #   两者都不算编造（红线未破），但会让用户看到"搜不到"，且违背"域内必真检索"。
        ...
        # 由代码补上可点击入口，用户看到的是"产品降级"而不是"功能坏了"。
