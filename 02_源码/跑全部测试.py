# -*- coding: utf-8 -*-
"""统一测试入口：一条命令跑完全部十二套测试，输出统一汇总表。

为什么需要它：
    本项目刻意采用「两版测试哲学」——
      · 离线注入版（冒烟 / 护栏 / 8 组验收 / 检索层解析 / 前端结构冒烟 / 澄清轮回归 /
        运行保障与网关边界 / 间接提示注入防御 / 编排层注入连线）：
        注入假模型与假检索，断言确定、可反复复跑（检索层解析用 fixtures/ 下的
        真实页面样本，仍属离线）；
      · 实网真机版（真实性取证 + 两轮对抗提示词）：真联网 + 真模型，出的是证据。
    各套测试因此都写成「脚本式证据工具」（各自打印明细表 + 退出码），
    而不是 pytest 函数式。本入口把它们串起来跑，给出一个统一结论。

用法：
    python 跑全部测试.py              # 全部十二套（含实网，需联网 + 已配密钥）
    python 跑全部测试.py --offline     # 跳过实网，只跑离线九套（确定、快、不需密钥）
...
def main() -> int:
    ...
    print("模式：%s" % ("仅离线九套（跳过实网取证与对抗测试）" if offline
                     else "全部十二套（离线九套 + 实网三套：真实性取证 + 两轮对抗提示词自测）"))
    print("=" * 78)
